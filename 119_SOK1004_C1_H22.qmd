...
```{r}
# Først tar jeg en av tabellene i "df", i dette tilfelle tabell 2, og lagrer dette som "df". Dette gjør jeg fordi de to tabellene lagrer den samme informasjonen bare med litt ulike variabelnavn. Dette trenger jeg ikke for å løse arbeidskravet.   

df <- df[[2]]


# Deretter lagrer jeg "Tid" (årstallene) som <int> i steden for <chr>.

df <- df %>%
  mutate(Tid=parse_number(Tid))


# Så endrer jeg variabelnavnene til "var", tid" og "verdi".  

df <- df %>%
  rename(var = ContentsCode, tid = Tid, verdi = value)


# Til slutt lagrer jeg "df" som en tibble. 
...
# Først lager jeg en figur som viser utviklingen i BNP (per innbygger) fra 2000 til 2021 målt i løpende priser. 

df %>%
  filter(var == "BNP") %>%
  filter(tid %in% 2000:2021) %>%
  ggplot(aes(x=tid,y=verdi)) +
  geom_line(color = "purple") +
  labs(title= "BNP (NOK) per innbygger målt i løpende priser 2000 - 2021", x = " ", y = "NOK per innbygger")


# Deretter lager jeg en figur som viser utviklingen i BNP (per innbygger) fra 2000 til 2021 målt i faste priser. 
...
df %>%
  ...
  labs(title="BNP (NOK) per innbygger målt i faste priser 2000 - 2021", x = " ", y = "NOK per innbygger")


# Forskjellen når man ser på BNP målt i faste og løpende priser er at BNP målt i løpende priser justeres ikke for reelle priser. Sagt på en annen måte tar ikke BNP målt i løpende priser hensyn til kjøpekraft. Om den nominelle inntekten til innbyggerne går opp vil dette ha liten effekt om prisene også går opp. Altså ingen reel inntektsvekst. 

# Sett i forhold til både nominell og reel vekst kan man si at inntekten i landet har økt siden år 2000. Om man ser på den nominelle veksten i BNP får man et inntrykk av at det har vært en ganske jevn vekst siden 2000. Hendelser som finanskrisen (2008), oljeprisnedgangen (2014) og koronapandemien viser mindre nedganger, men ikke av en veldig dramatisk art. Der i mot om man ser på grafen som viser utviklingen i reelt BNP kan man tydelig se den negative innvirkningen finanskrisen i 2008 har hatt på norsk økonomi. Oljeprisfallet har i motsetning nesten ikke hatt en synlig effekt på inntekten i Norge reelt sett. Man ser også at koronapandemien satte norsk økonomi noe tilbake, men både den reele og nominelle veksten har vært tydelig i kjølevannet av pandemien. 
```
...
[*Svar:*]{.underline}*\
Koden pivot_wider omorganiserer tabellen og lager den "videre". Det vil si at koden tok variabelnavnene "BNP" og "MEMOBNP" fra kolonnen "var" og lagde i steden to nye kolonner, henholdsvis BNP og MEMOBNP. Koden tar også verdiene fra kolonnen "verdi" og fordeler det mellom "BNP" og "MEMOBNP", alt etter hvilken variabel verdien hører til.*
...
```{r}
# Ovenfor er det blitt lagt til kolonnene "LBNP" og "LMEMOBNP". Nå bruker jeg verdiene i disse kolonnene og funksjonen "mutate" til å lage to nye kolonner, henholdsvis DBNP og DMEMOBNP, som viser den relative endringen fra t-1 til til t. I disse kolonnene vises verdiene som beregnes ut i fra formlene 100*(delta X/Xt-1), altså relativ endring i BNP/innbygger både målt i faste (MEMOBNP) og løpende (BNP) priser.  
...
```{r}
# Først lager jeg en figur som viser prosentvis vekst i nominellt BNP (BNP målt i løpende priser):

df_long %>%
  filter(var == "DBNP") %>%
  filter(tid %in% 1970:2021) %>%
  ggplot(aes(x=tid,y=verdi)) +
  geom_line(color = "blue") +
  geom_point(color = "dark blue") +
  geom_smooth(color = "light blue") +
  labs(title= "Prosentvis endring i nominellt BNP (NOK) per innbygger 1970 - 2021", x = " ", y = "prosentvis endring")


# Deretter lager jeg en figur som viser prosentvis vekst i reelt BNP (BNP målt i faste priser):
...
df_long %>%
  ...
  labs(title= "Prosentvis endring i reelt BNP (NOK) per innbygger 1970 - 2021", x = " ", y = "prosentvis endring")


# I grafen som er laget ovenfor ("Prosentvis endring i reelt BNP (NOK) per innbygger 1970 - 2021") er det tre tidperioder som står ut hvor den reele økonomiske veksten er negativ. 

# I perioden 1986-1987 kan man se en betydelig nedgang i den reele økonomiske veksten, hvor den på sitt laveste har negativ økonomisk vekst. Man kan anta at dette skylles slutten på den såkaldte "Jappetiden". 

# Den høyeste, negative reele veksten kan man observere rundt 2008-2009 hvor man kan se at veksten er på under -2,5 prosent. Dette skyldes høyst sannsynlig Finanskrisen i 2008.

# Den siste perioden hvor man kan observere negativ reel vekst er i 2020. Dette kan man i veldig sannsynlig grad knytte opp mot koronapandemien sitt første år, som var akkurat 2020. Da gikk flere, spesielt mindre, bedrifter dårlig og/eller konkurs som resultat av nedstengingen i landet. 
```
